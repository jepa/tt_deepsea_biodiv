...
run_biodiv_analysis <- function(df_subset) {
      ...
      iNEXT(inc_freq, q = 0, datatype = "incidence_freq", nboot = 1, endpoint = 50) # Benthic is 125 and pelagic is 250
}
...
benthic_df <- as.data.frame(results_list$Benthic$iNextEst$size_based) %>%
  mutate(Group = "Benthic") %>% 
      filter(t < 125) #just for plotting 
...
# delta <- Hills_q0_panel_df %>%
#   arrange(t) %>%
#       group_by(Group,Method) %>% 
#   mutate(delta = c(NA, diff(qD))) %>%   # change in richness between steps
#   filter(abs(delta) > 10) 
...
sb_se <- results_list$Benthic$AsyEst$Est_s.e.[1] #76.3628

# For Pelagic species
sp_obs <- results_list$Pelagic$AsyEst$Observed[1] # 137
sb_se <- results_list$Pelagic$AsyEst$Est_s.e.[1] #226.61
...
inc_freq <- as.incfreq(as.matrix(t(community_matrix_pres)))
Hills_q_0_inc_CCZ <- iNEXT(inc_freq, q = 0, datatype = "incidence_freq", nboot = 1, endpoint = 50) # q and qdp is what plots
...
bind_rows(Hills_q0_inc_df, Hills_q0_panel_df) %>% 
  mutate(Group = ifelse(is.na(Group), "Family", Group),
         Group = factor(Group, levels = c("Family", "Benthic", "Pelagic"))) %>% 
  ggplot() +
