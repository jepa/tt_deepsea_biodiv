...
run_biodiv_analysis <- function(df_subset) {
       ...
       # Guard: skip empty subsets
  if (nrow(df_subset) == 0) {
    return(NULL)  # or a default tibble with NA/zero estimates
  }
      ...
      # Also guard very small matrices (e.g. 0 sites)
  if (nrow(community_matrix_pres) == 0) {
    return(NULL)
  }
      iNEXT(inc_freq, q = 0, datatype = "incidence_freq", nboot = 1, endpoint = 22) # As pointed out by reviewer 
}
...
# Split and run analysis by benthic/pelagic group
results_list <- spp_biodiv_df %>%
      group_by(depth_boxes) %>%
      group_split() %>%
      set_names(unique(spp_biodiv_df$depth_boxes)) %>%
      map(run_biodiv_analysis) %>%  # or imap() to add error handling
  keep(~ !is.null(.x))  # drop empty results
